...
面向个人用户的 AI 图片生成与编辑工作台。`v0.6.1` 是 `v0.6.0` 自动提示词分发与工作台图片预览体验的桌面缓存 hotfix：API、Worker、Web 发布为 GHCR Docker 镜像，Windows portable 包是 Electron 桌面窗体应用；桌面壳会依赖 Docker Desktop 运行本地服务栈，用户不需要在本机执行 `pnpm install`。
...
- Windows portable Electron 桌面壳：基于 Docker Desktop 拉取 v0.6.1 镜像运行本地服务栈，并可检查 GitHub Release 更新
...
- `.github/workflows/release.yml`：发布工程 workflow
- `docs/release/v0.6.1.md`：当前版本发布说明
...
Release artifact 中的 `Yunwu.Desktop-0.6.1-win-x64-portable.zip` 是 Electron 桌面窗体应用。解压后运行桌面程序，桌面壳负责检查 Docker CLI/daemon，并使用内置 compose 文件拉起 API、Worker、Web、PostgreSQL、Redis、MinIO。

如需不经过桌面壳、直接验证同一组 v0.6.1 镜像，可手动运行：
...
- 桌面版关闭后容器仍运行：请升级到 `v0.6.0` 或更新版本；桌面壳退出时会执行当前实例的 `docker compose down`，保留数据卷。
...
- 上传或结果图片不可访问：检查 MinIO 是否健康、`MINIO_BUCKET` 是否初始化；`v0.6.0` 或更新版本会按当前实例端口代理本地资产，不依赖旧端口。
...
- v0.6.0 发布说明：[docs/release/v0.6.0.md](docs/release/v0.6.0.md)
